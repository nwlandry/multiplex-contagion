...
def plot_multiplex_network(ax, networks, pos, gc_colors, iso_colors, edge_colors, plane_colors, labels, node_size=5, edge_width=0.5, width=8, height=6, num_thru_nodes=10, label_pos=0.8):
    if ax.name != "3d":
    ...
    for gi, G in enumerate(networks):
        ...
        if gi > 0:
            thru_nodes = np.random.choice(list(G.nodes()), num_thru_nodes, replace=False)
        ...
        line_collection = Line3DCollection(lines3d, zorder=gi, color=edge_colors[gi], alpha=0.8, linewidth=edge_width)
        ax.add_collection3d(line_collection)
        
        # now add GC nodes
        ax.scatter(xs, ys, zs, c=cs, s=node_size, edgecolors='.2', linewidth=0.2, marker='o', alpha=1, zorder=gi+1)
        ...
        ax.text(0.0, x_pos, gi*0.95+0.5, labels[gi], color='black', fontsize='large', zorder=1e5, ha='center', va='center')
